...
This webpage aims to provide the commented R code and the datasets to reproduce all the analyses and results described in the main paper  (*Literature reference, when available*). We also give some extra hints, to open up other possibilities, not considered in the paper. Our hope is that, whenever censored data are recognised, the correct techniques are used more extensively. 
